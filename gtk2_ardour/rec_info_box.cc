...
RecInfoBox::render (Cairo::RefPtr<Cairo::Context> const& cr, cairo_rectangle_t* r)
{
	...
	samplecnt_t  capture_duraion = _session->capture_duration ();
	...
	if (recording || capture_duraion == 0) {
	...
	if (capture_duraion > 0) {
		/* compare to AudioClock::print_minsec */

		samplecnt_t left = capture_duraion;
		int hour = (int) floor (left / (sample_rate * 3600.f));
		left -= (samplecnt_t) floor (hour * sample_rate * 3600.f);
		int mins = (int) floor (left / (sample_rate * 60.f));
		left -= (samplecnt_t) floor (mins * sample_rate * 60.f);
		int secs = (int) floor (left / (float) sample_rate);
		left -= (samplecnt_t) floor ((double)(secs * sample_rate));
		int decs = floor (left * 10.f / (float) sample_rate);

		char buf[32];
		snprintf (buf, sizeof(buf), "%02" PRId32 ":%02" PRId32 ":%02" PRId32 ".%01" PRId32, hour, mins, secs, decs);
		_layout_value->set_text (buf);
	} else {
